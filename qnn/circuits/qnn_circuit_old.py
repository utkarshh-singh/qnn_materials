...
def _make_qnnc(
    ...
    feature_map,
    ansatz,
    fm_kwargs: Dict[str, Any] | None,
    ansatz_kwargs: Dict[str, Any] | None,
) -> QNNCircuit:
...
def _capture_params(qnnc: QNNCircuit) -> tuple[list, list]:
    # Preserve the split BEFORE lowering to plain QuantumCircuit
...
def build_qnn_raw(
    ...
    *,
    ...
    """
    Build QNN circuit and return (qc, input_params, weight_params).
    Set decompose=False if you want to keep library instructions (e.g., for visualization).
    """
...
def build_qnn_for_runtime(
    ...
    ansatz_kwargs: Optional[Dict[str, Any]] = None,
    *,
    backend=None,                 # <--- NEW: pass the IBM backend here
    optimization_level: int = 1,
    seed_transpiler: int = 42,
    pass_manager=None,
    target=None,
    ...
    """
    Transpile the circuit against a concrete backend (preferred) or, if not provided,
    apply a preset pass manager built for 'target'.
    """
    ...
    if backend is not None:
        ...
        # Fallback: use a preset pass manager (supply 'target' if you have it)
